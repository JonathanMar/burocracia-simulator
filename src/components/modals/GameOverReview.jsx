          ...
          <div key={i} style={{background:"#0f0a0a",border:"1px solid #2a1a1a",borderRadius:6,padding:"10px 14px",marginBottom:10}}>
            ...
            {doc.issues.length > 0 ? (
              <div>
                ...
                {doc.issues.map(iss=>(<div key={iss} style={{color:"#cc7744",fontSize:10,marginLeft:8}}>• {iss}</div>))}
              </div>
            ) : (
...
export default function GameOverReviewWrapper({ wrongDocs, grade, gc, score, stats, unlocked, onStartGame, onMenu, onEndless }) {
  ...
  const canEndless = grade === 'S' || grade === 'A';
  return (
    <div style={{textAlign:"center",maxWidth:480,padding:40}}>
      ...
      <div style={{background:"#080812",border:"1px solid #1a1a2e",borderRadius:8,padding:20,marginBottom:18}}>
        ...
        <div style={{background:"#0a0a08",border:`1px solid ${salaryFinal>0?"#336633":"#663333"}`,borderRadius:5,padding:"10px 16px",marginBottom:14}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr auto",gap:"4px 12px",fontSize:10}}>
            ...
            {salaryDeductions > 0 && <>
              <span style={{color:"#777",textAlign:"left"}}>Descontos ({stats.errors} erros)</span>
      ...
      <div style={{display:"flex",gap:10,justifyContent:"center",flexWrap:"wrap"}}>
        ...
        {canEndless && onEndless && (
          <button onClick={onEndless} style={{background:"#1a4a1a",border:"1px solid #336633",color:"#55cc55",padding:"10px 22px",borderRadius:4,cursor:"pointer",fontFamily:"'Courier New',monospace",fontSize:12,fontWeight:"bold"}}>▶ CONTINUAR (Dia 5+)</button>
        )}
